...
//	private Combo comboOrientation;
	...
	private Image image;
	private IPrintImageProvider printImageProvider;
	
	private String fileName = "Dawn plot";
	 ...
	 */
	public PlotPrintPreviewDialog(IPrintImageProvider xyGraph, Display device, PrintSettings settings) {
		...
		this.printImageProvider = xyGraph;
	...
	private Rectangle getImageSizeRect(IPrintImageProvider xyGraph){
	...
	private SelectionAdapter aspectRatioListener = new SelectionAdapter(){
		...
		public void widgetSelected(SelectionEvent e) {
			...
			if(buttonAspectRatio.getSelection()){
				image = printImageProvider.getImage(getImageSizeRect(printImageProvider));
			}else {
				image = printImageProvider.getImage(getPrinterSizeRect(printer));
			}
			canvas.redraw();
		}
	};
